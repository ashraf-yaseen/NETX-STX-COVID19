...
pdf_folder = 'C:/Users/Yashar/Google Drive/UT HSC/Projects/COVID_19/PDFtoText/Scanned_Documents/'
...
output_folder = 'C:/Users/Yashar/Google Drive/UT HSC/Projects/COVID_19/PDFtoText/Scanned_Documents/'
### define the path to tessaract.exe that will be used by pytessaract (OCR)
pytesseract.pytesseract.tesseract_cmd = r'C:\Program Files\Tesseract-OCR\tesseract.exe'

### setiing up google geolocater ###
api_key = 'AIzaSyCmwRiALb9nFB438K_fRZoLNZRc0SyZZrQ' #####Put your API key here
...
#scanned_page_processing function gets the page object and reference_page_structure
#and first fixes the rotation of the page and second calculates  x and y offset and scale
#for the page and returns page and transformation_matrix as a tuple
def scanned_page_processing (page,
                             reference_page_structure,
                             width=4250,
                             height=5500):
    ############# fixing page rotation #########################
    img = preprocess_image(page, preprocess = "detect rectangle")
...
# =============================================================================
#     This is for situation we have varying image sizes
#     Xs = [i[0] for i in box]
#     Ys = [i[1] for i in box]
#     x1 = min(Xs)
#     x2 = max(Xs)
...
# =============================================================================
#     this is for rotation more than 45 degress
#     rotated = False
#     if angle < -45:
#         angle += 90
#         rotated = True
# =============================================================================
    ...
    croppedW = width #if not rotated else height
    croppedH = height #if not rotated else width
    
    image = cv2.getRectSubPix(
        cropped, (int(croppedW*scale), int(croppedH*scale)), (size[0]/2, size[1]/2))
    
    
    ############# claculating page scale and offset #########################
    thresh = cv2.threshold(image, 195, 255, cv2.THRESH_BINARY_INV)[1]
    
    # find contours
    contours = cv2.findContours(thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[0]
    
    mask = np.ones(image.shape[:2], dtype="uint8") * 255
    current_page_structure = {"box_numbers" : reference_page_structure["box_numbers"],
                               "xs" : [],
                               "ys" : [],
                               "ws" : [],
                               "hs" : [],
    ...
    for c in contours:
        ...
        #600000 is the minimum surface size of the detected box. We are only interested
        #on bigger ones
        if w*h>600000:
            current_page_structure["xs"].append(x)
            current_page_structure["ys"].append(y)
            current_page_structure["ws"].append(w)
            current_page_structure["hs"].append(h)
    number_of_calculations = current_page_structure["box_numbers"]
    
    xs_list, xo_list, ys_list, yo_list = ([] for i in range(4))
    for calculation_number in range(number_of_calculations):
        xsmp = current_page_structure["xs"][calculation_number]
        ysmp = current_page_structure["ys"][calculation_number]
        wsmp = current_page_structure["ws"][calculation_number]
        hsmp = current_page_structure["hs"][calculation_number]
        xref = reference_page_structure["xs"][calculation_number]
        yref = reference_page_structure["ys"][calculation_number]
        wref = reference_page_structure["ws"][calculation_number]
        href = reference_page_structure["hs"][calculation_number]
    ...
    xscale_average = sum(xs_list)/len(xs_list)
    xoffset_average = sum(xo_list)/len(xo_list)
    yscale_average = sum(ys_list)/len(ys_list)
    yoffset_average = sum(yo_list)/len(yo_list)
    xs_list_final, xo_list_final, ys_list_final, yo_list_final = ([] for i in range(4))
    for location in range(number_of_calculations):
        if abs(xo_list[location]-xoffset_average)<4:
            xo_list_final.append(xo_list[location])
        if abs(xs_list[location]-xscale_average)<0.005:
            xs_list_final.append(xs_list[location])
        if abs(yo_list[location]-yoffset_average)<4:
            yo_list_final.append(yo_list[location])
        if abs(ys_list[location]-yscale_average)<0.005:
            ys_list_final.append(ys_list[location])
    
    xscale_average = sum(xs_list_final)/len(xs_list_final)
    xoffset_average = sum(xo_list_final)/len(xo_list_final)
    yscale_average = sum(ys_list_final)/len(ys_list_final)
    yoffset_average = sum(yo_list_final)/len(yo_list_final)
    current_page_structure["xscale"] = (xscale_average + yscale_average)/2
    current_page_structure["xoffset"] = xoffset_average
    current_page_structure["yscale"] = (xscale_average + yscale_average)/2
    current_page_structure["yoffset"] = yoffset_average
    return (image,
            (current_page_structure["xscale"],
             current_page_structure["xoffset"],
             current_page_structure["yscale"],
             current_page_structure["yoffset"]),
            current_page_structure)



#element_new_cordinates funstion is used to calculate new coordinates for each element
#in page. Only use it when the file is sccaned PDF
def element_new_cordinates(old_coordinates,
                           transformation_matrix,
                           shrinkage=2):
    result = (int((old_coordinates[0]*transformation_matrix[0])+transformation_matrix[1]+shrinkage),
              int((old_coordinates[1]*transformation_matrix[2])+transformation_matrix[3]+shrinkage),
              int((old_coordinates[2]*transformation_matrix[0])+transformation_matrix[1]-shrinkage),
              int((old_coordinates[3]*transformation_matrix[2])+transformation_matrix[3]-shrinkage))
    return result
...
def text_box_process (page,
                      field_value,
                      transformation_matrix=None):
    if transformation_matrix:
        crop_coordinates = element_new_cordinates(old_coordinates = field_value[1],
                                                  transformation_matrix=transformation_matrix)
    else:
        crop_coordinates = field_value[1]
    cropped=page.crop(crop_coordinates)
...
def radio_button_process (page,
                          field_value,
                          background_mean,
                          transformation_matrix=None):
    result = None
    current_choice = background_mean-(background_mean//15)
    for index, option in enumerate(value[3]):
        if transformation_matrix:
            crop_coordinates = element_new_cordinates(old_coordinates = field_value[1][index],
                                                      transformation_matrix=transformation_matrix)
        else:
            crop_coordinates = field_value[1][index]
        cropped=page.crop(crop_coordinates)
        preprocessed_image = preprocess_image(cropped)
...
def check_box_process (page,
    ...
    if field_value[0] == "Student":
        ...
        if box_mean < background_mean-(background_mean//15):
            result = field_value[3][0]
    return result
...
#reference_structure is the dictionary that have the necessary information from
#reference structure (saved pdf files) so that scanned_page_processing function
#can calculate x and y offset and scale for the scanned pdf files and fix the rotation.
reference_structure = {0:{"box_numbers" : 3,
                          "xs" : [286,288,288],
                          "ys" : [2889,1004,663],
                          "ws" : [3748,3713,3713],
                          "hs" : [1636,534,269]},
                       1:{"box_numbers" : 4,
                          "xs" : [295,286,2286,249],
                          "ys" : [3855,2914,2914,1494],
                          "ws" : [3786,1810,1810,3752],
                          "hs" : [816,660,625,833]}}
...
for pdf_filename in glob.glob(f'{pdf_folder}*.pdf'):
    ...
    number_of_forms = int(len(doc)/2)
    for form_number in range(number_of_forms):
        df.at[row,'File Name'] = f'{os.path.basename(pdf_filename)[:-4]}_form_{form_number}'
        print (f'{os.path.basename(pdf_filename)[:-4]}_form_{form_number}')
        page_number = 0
        for page_number, page_structure in enumerate(pdf_structure.values()):
            ...
            ### PIL Image as Object
            current_page = doc.loadPage((form_number*2)+page_number)
            ...
            processed_page, transformation_matrix, check = scanned_page_processing(page, reference_structure[page_number])
            # The page result of scanned_page_processing is a numpy array. The following lines
            # are used to load it back as PIL.PngImagePlugin.PngImageFile
            tm.append(transformation_matrix)
            st.append(check)
            page_image = Image.fromarray(processed_page, mode=None)
            page_image_RGB = page_image.convert("RGB")
            buffer = io.BytesIO()
            page_image_RGB.save(fp=buffer, format='PNG', mode="RGB")
            page = Image.open(buffer)
            background_picture = page.crop(element_new_cordinates(old_coordinates = (2600,5200,2800,5400),
                                                          transformation_matrix=transformation_matrix,
                                                          shrinkage=0))
            background_mean = np.mean(preprocess_image(background_picture))
            #page.save(f'{output_folder}test1page1.png')
            #print ("page no: ", page_number)
            for key, value in page_structure.items():
                #print (key)
                if value[2]=="text box":
                    df.at[row,value[0]] = text_box_process(page,
                                                           value,
                                                           transformation_matrix)
                if value[2]=="radio button":
                    df.at[row,value[0]] = radio_button_process(page,
                                                               value,
                                                               background_mean,
                                                               transformation_matrix)
                if value[2]=="check box":
                    df.at[row,value[0]] = check_box_process(page,
                                                            value,
                                                            background_mean,
                                                            transformation_matrix)
                if value[2]=="digit box":
                    df.at[row,value[0]] = digit_box_process(page,
                                                            value,
                                                            transformation_matrix)
...
center = (int(4250/2), int(5500/2))
size = (int(4250), int(5500))
...
cv2.imshow("final image", page)
cv2.waitKey(0)
cv2.destroyAllWindows()
tm[0]
pdf_structure["Page 1"][93][1]
t_fever = element_new_cordinates(old_coordinates = (577, 2445, 615, 2484), #pdf_structure["Page 2"][1][1][1],
                           transformation_matrix=tm[0])
t_his = element_new_cordinates(old_coordinates = (2894, 4858, 2933, 4897), #pdf_structure["Page 2"][1][1][1],
                           transformation_matrix=tm[0])