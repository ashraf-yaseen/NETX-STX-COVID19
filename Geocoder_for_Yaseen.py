# -*- coding: utf-8 -*-
"""
Created on Tue Jul 28 15:24:16 2020
...
def google_response_by_address (geo_data_path,
    ...
    for row in range(len(geo_data)):
        ...
        if result.standard_address_zip_code_long=="":
            result = Google_geocoder_address (gps_id = geo_data.at[row, id_field_name],
                                          street = str(geo_data.at[row, street_field_name]),
                                          city = str(geo_data.at[row, city_field_name]),
                                          state = str(geo_data.at[row, state_field_name]))
...
xl = pd.ExcelFile('C:/Users/Yashar/Google Drive/UT HSC/Yaseen/CPL Texas PSC Locations 07142020.xlsx')
xl.sheet_names
df = xl.parse("report")
df.to_csv('C:/Users/Yashar/Google Drive/UT HSC/Yaseen/df.csv' , index = False, encoding = "utf-8")
api_key = 'AIzaSyCmwRiALb9nFB438K_fRZoLNZRc0SyZZrQ' #####Put your API key here
geolocator = GoogleV3(api_key = api_key, timeout=None)
t = pd.read_csv('C:/Users/Yashar/Google Drive/UT HSC/Yaseen/df.csv', header = 0, low_memory = False, encoding = "utf-8")

google_response_by_address (geo_data_path = 'C:/Users/Yashar/Google Drive/UT HSC/Yaseen/df.csv',
                            output_path = 'C:/Users/Yashar/Google Drive/UT HSC/Yaseen/',
                            id_field_name = 'Misys Room Number',
                            street_field_name = 'Address',
                            city_field_name = 'City',   
                            state_field_name = 'St',
                            postal_code_field_name = 'ZipCode',
                            save_reponse_object = False,
                            api_key = api_key)

import re
t = "6121 N STATE HWY 161 SUITE 101"
x = re.sub('SUITE', '', t)
re.search('SUIt', t).start()
t.lower()
t[:re.search('SUITE', t).start()-1]
for row in range(len(df)):
    address = df.at[row, 'Address']
    if re.search('suite', address.lower()):
        position = re.search('suite', address.lower()).start()
    if re.search('ste', address.lower()):
        position = re.search('ste', address.lower()).start()
    df.at[row, 'Address'] = address[:position-1]


t = Google_geocoder_address(1,
                            street = "6121 N STATE HWY 161 SUITE 101",
                            city = "IRVING",
                            state = "TEXAS",
                            postal_code = "75063",
                            country = "US",
                            one_line_address = None,
                            tries = 5)
pd.isnull(t)
t.standard_address_zip_code_long==""
